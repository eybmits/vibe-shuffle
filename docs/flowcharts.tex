...
% =========================================================================
% PAGE 1 — Heartbeat -> arousal level
% =========================================================================
\colorlet{accent}{hrv}
\charttitle{How your heartbeat becomes an Arousal level}{}

\begin{center}
\begin{tikzpicture}[node distance=8mm]
  \node[box] (b1)
    {\head{A chest strap measures your heartbeat}
     We read heart rate and the time gaps between consecutive beats.};
  ...
  \node[box, below=of b2] (b3)
    {\head{While a song plays, we compare to that resting rhythm}
     Higher heart rate and lower short-term variability raise Arousal; the
     opposite changes lower it. Small changes are ignored as noise.};
  \node[box, fill=face!12, draw=face, line width=1pt, below=of b3] (bmove)
    {\head{Your movement is always added on top}
     The more you move and bob along to the music, the higher the Arousal~--- and
     this counts \emph{even while the heart strap is active}.};
  \node[below=8mm of bmove] (bar) {\scalebar{relaxed}{hrv}{Low Arousal}{High Arousal}};
  \node[result, below=8mm of bar] (b5)
    {\head{Result: an Arousal level}
     Your heartbeat moves Arousal up or down, and moving along always pushes
     it further up.};
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \node[box, below=27mm of a2] (a3)
    {\head{Curate 25 popular, unique songs in each corner}
     Freeze the result as one balanced pool of \textbf{100 songs}.};
  ...
  \node[result, below=of a3] (b1)
    {\head{Your average mood during the song lands in one corner}
     ...
     your heartbeat sets the \emph{Arousal} (low\,$\leftrightarrow$\,high).
  ...
  \node[mode, fill=song!10, draw=song, text width=6.1cm,
    ...
    {\textbf{Vibe mode}\\[2pt]
     Keep unplayed songs in the matching corner, then pick the one with the
     shortest distance to your average 60 s position.};
  \node[mode, text width=6.1cm, below=11mm of b1, xshift=3.6cm] (rand)
    {\textbf{Random mode}\\[2pt]
     Pick from all remaining songs using the session's seeded random order;
     measured signals are ignored.};

  \node[box, below=42mm of b1] (pick)
    {\head{Validation}
     Five songs run in each of two masked blocks. Every song plays for up to
     60 s and receives liking, mood-fit, and mood ratings.};
